...
export function SongDetailModal(isOpen: boolean, onClose: () => void) {
	const dispatch = useAppDispatch();
	const { currentSong, timeElapsed, isActive } = useAppSelector(
		(state) => state.player
	);
						...
						<div className="mx-4 my-8 pt-2 lg:pt-0 flex justify-start w-full lg:w-1/3">
							<div>
								...
								<div className="flex flex-col justify-center m-1 lg:m-4">
									<h6
										id="song-name"
										className="text-white font-semibold text-3xl text-left">
										{currentSong.name}
									</h6>
								...
								<Link href="">
									<div className="p-2">
										<Image
											src={currentSong.picture_url}
											alt=""
											className="m-1 rounded-full w-32 h-32"
										/>
