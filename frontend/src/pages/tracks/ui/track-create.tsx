...
import { TrackpackService } from '@/services';
...
export function TrackCreate() {
	const [Track, setTrack] = useState<TrackResponseType[]>([]);
	const [loading, setLoading] = useState(true);

	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await TrackService.get_one(id);
				const responseData = response.data;
				setTrack(responseData);
				setLoading(false);
			} catch (error) {
				console.error(error);
			}
		...
		<MainLayout>
				...
				<div className="hidden mt-8 flex items-center md:flex">
					...
					<div className="mt-16">
						<h2 className="text-gray-50 uppercase text-md font-semibold tracking-tighter mr-2 mt-1">
							Track by {data.prod_by} (ww/ {data.co_prod})
						</h2>
			...
			<div className="w-full ">
				<div className="w-full p-4">
					...
					<p>
						You can to lease or purchase Track for any license, and rules will be
						you
					</p>
					<div className="flex flex-wrap">
						<LicenseLink
							...
							description="you get a ahuenni Track"
						/>
						<LicenseLink
							link=""
							image={data.picture_url}
